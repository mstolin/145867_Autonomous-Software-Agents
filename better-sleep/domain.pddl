...
; WAS NOCH FEHLT:
; 1. Action Turn-off-light
; 2. Bedingung: Wenn eine person ein raum betritt, licht an!
; move precondition: (light-on z) (not (light-on y))
...
(define (domain better-sleep)
    ...
    (:action move ;Move person x from room y to room z
        ...
        :precondition (and (PERSON ?x) (ROOM ?y) (ROOM ?z)
                      (can-move ?y ?z) (in-room ?x ?y) (not (in-room ?x ?z)))
        ;x is not in room y, x is in room z
        :effect (and (not (in-room ?x ?y)) (in-room ?x ?z)) ; !!! MAYBE REMOVE THE NOT IN ROOM
    )
)
