...
function setup(intentions = []) {
    var PlanningIntention = class extends Intention {
        ...
        async blackboxExec(domainFile, problemFile) {
            ...
            var child = execFile("./bin/blackbox", [
                "-o",
                domainFile,
                "-f",
                problemFile,
            ...
            for (let line of result.split("\n")) {
                // this.log(line)
                if (line == "Begin plan") planBeginned = true;
                else if (line == "End plan") break;
            ...
            for (let line of planStruct) {
                ...
                for (
                    let index = 0;
                    index < intentionClass.parameters.length;
                    index++
                ) {
                ...
                var intentionInstance = new intentionClass(
                    this.agent,
                    new pddlActionGoal(mappedArgs)
                );
                this.plan.push({
                    parallel: number == previousNumber,
                    intention: intentionInstance,
                });
            }
        ...
        *exec() {
            ...
            for (const step of this.plan) {
                if (step.parallel) {
                    this.log(
                        "Starting concurrent step with Intention: " +
                            step.intention.toString()
                    );
                } else {
                    yield Promise.all(previousStepGoals);
                    previousStepGoals = [];
                    this.log(
                        "Starting sequential step with Intention: " +
                            step.intention.toString()
                    );
                }
                previousStepGoals.push(
                    step.intention.run().catch((err) => {
                        return Promise.reject(
                            new Error("Plan execution aborted")
                        );
                    })
                );
            }
