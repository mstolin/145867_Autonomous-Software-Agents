    ...
    var OnlinePlanning = class extends Intention {
        ...
        async doPlan(domainFile, problemFile) {
            ...
            var res = await fetch("http://solver.planning.domains/solve", {
                ...
                body: JSON.stringify({
                    domain: domainFile.content,
                    problem: problemFile.content,
                }),
            })
                .then(function (res) {
                    return res.json();
            ...
            for (let line of planStruct) {
                ...
                this.plan.push({
                    parallel: false /*number==previousNumber*/,
        ...
        *exec() {
            ...
            for (const step of this.plan) {
                if (step.parallel) {
                    this.log(
                        "Starting concurrent step " + step.intention.toString()
                    );
                } else {
                    yield Promise.all(previousStepGoals);
                    previousStepGoals = [];
                    this.log(
                        "Starting sequential step " + step.intention.toString()
                    );
                }
